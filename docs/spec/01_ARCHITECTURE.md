...
**A leitura do store é total.** `read_all` devolve todo evento gravado, sempre. **Nenhum caminho de leitura compartilhado filtra por epoch, por abandono ou por ponto de corte de rollback.** A exclusão dos eventos de epoch abandonada posteriores ao corte vive **no escopo de quem reconstrói o mundo corrente** — nunca no store, nunca em uma consulta que as outras projeções herdem.

São dois consumidores hoje: o **fold de `simulation_state`**, que reconstrói as flags, e o **avaliador de predicados de verificação** (`03_EXERCISE_DESIGN.md` §3.1), que decide se contenção e restauração valem **agora**. Consumidor novo que precise do mundo corrente se enquadra pela classe; a lista é registro do que há, e não a fonte.

**A linhagem corrente tem UMA definição, e ela é função pura nomeada.** Os dois consumem a mesma. Duas implementações de *"quais escritas sobrevivem ao corte"* divergiriam em silêncio — e no pior lugar possível: o fold diria que a flag voltou atrás e o avaliador diria que o ato continua valendo, que é precisamente o **predicado meio-revertido** que esta seção existe para tornar inexprimível. A unicidade é norma, e não estilo.

**O avaliador recebe o fluxo total mais a escrituração de epoch, e a consciência de linhagem é lógica dele** — não recorte de quem monta o insumo. É a mesma forma que `00_MASTER_SPEC.md` §3.2 exige do desconto por união e da exclusão de `rehearsal`: *"o número certo aparecendo por ausência de insumo em vez de por cálculo"* é o defeito que aquela seção nomeia, e ele vale igual aqui. A consequência é testável, e é isso que a distinção compra — existe teste que planta um rollback atravessando o ato e afirma que o predicado **dessatisfaz**, o que só é possível porque a lógica está no avaliador.

> **A frase anterior nomeava a instância, e nasceu verdadeira.** Ela dizia *"vive no fold de `simulation_state`, e em lugar nenhum mais"*, e era exata enquanto o fold era o único a reconstruir o mundo corrente — nasceu com ele. Envelheceu quando o avaliador chegou, e o **modo** como envelheceu importa: a leitura literal proibiria exatamente o que o avaliador precisa fazer para não estar errado. É a classe que o registro da Fase 1 chama de §1.6 — a afirmação que envelhece depois de correta —, e a correção é generalizar de instância para classe, não trocar uma instância por duas.
>
> **O que continua proibido é o que sempre esteve:** filtro no **caminho compartilhado de leitura**, que é o que faria as outras projeções herdarem uma perda que nenhuma escolheu — e é o que `read_all` sem parâmetro impede.

O motivo é que **quatro das cinco projeções leem a epoch abandonada legitimamente**, cada uma pelo motivo declarado em `09_EVENT_MODEL.md` §3.1: `rehearsal` descarta a epoch do cálculo, `technical_failure` desconta o intervalo, `facilitation` e `adjudication` preservam a epoch anterior — e `aar_timeline` a renderiza com anotação de rollback. Das **cinco projeções**, só `simulation_state` é reconstruída, e é a única que o diagrama acima marca. O avaliador de predicados **não é projeção**: ele consome o fluxo e **emite**. Por isso não entra na conta das cinco, e por isso a regra que o alcança é a da classe acima, e não a do diagrama.
...
**A persona vê pelo domínio e declara pelo núcleo.** Os painéis de `03_EXERCISE_DESIGN.md` §6 são telas do adapter — `domains/<adapter>/panels/` —, e é a tela do adapter que **chama** esta superfície. A separação não é organizacional: **nenhuma rota de declaração de `03_EXERCISE_DESIGN.md` §3.4 vive no adapter**, e o verificador de superfície sustenta essa topologia recusando `persona` como **papel de rota** no perfil de domínio — o que autoriza uma rota do adapter é papel de domínio, nunca persona. Uma rota de declaração no adapter exigiria desfazer essa guarda.

**A guarda protege a topologia da declaração, e não a palavra.** `persona` no **envelope** de um evento que o adapter emite é legítimo, e é o que `09_EVENT_MODEL.md` §1 exibe como exemplo normativo do envelope universal: `producer: academus-api` com `persona: ti`, num `audit_query_performed`. O que decide é a **família do ato**, e `03_EXERCISE_DESIGN.md` §3.4 já a fixa: **ato de declaração** — afirmação sobre o incidente — é do núcleo, e **ato de observação** — `inject_viewed`, `audit_query_performed`, `evidence_source_opened` — tem rota própria, a da consulta de auditoria sendo da `academus-api` pela mesma seção. O adapter que registra **quem** observou não passa a conhecer desenho de exercício: ele carimba no envelope o ator que `09_EVENT_MODEL.md` §1.1 torna obrigatório na camada `participant_action`. Persona como identidade que viaja para o envelope: **permitida**. Persona como critério de autorização de rota do adapter: **proibida**.

> A forma anterior desta frase afirmava, sem qualificação, que **"`domains/` não pode conhecer persona"**, e trazia a topologia só no exemplo seguinte — *"uma rota de **declaração** no adapter"*. Lida isolada, ela proíbe o que `09_EVENT_MODEL.md` §1 exibe como normativo; e foi lida isolada **duas vezes**. Por `scripts/check_api_surface.py`, que pôs `persona` no vocabulário proibido de **claim** do perfil de domínio — quando o que a topologia exige é proibi-la como **papel**. E pela implementação da Fase 6, que emitiu `audit_query_performed` **sem** `persona`: envelope que `contracts/events.schema.yaml` recusa — o exemplo positivo daquele arquivo é este evento, com este produtor e com `persona` —, gravado num store append-only, onde não se corrige retroativamente. Corrigido escrevendo a qualificação que só existia no contexto. O que muda é a precisão da fronteira, não o lugar dela: as nove declarações de `03` §3.4 continuam no núcleo, e nenhuma rota de declaração passa a ser admissível no adapter.
